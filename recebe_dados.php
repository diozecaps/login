...
if(isset($_POST['action'])){
    if($_POST['action'] == 'cadastro'){
        ...
        if($senhaCodificada != $senhaConfirmarCod){
            ...
            if($linha['nomeDoUsuario'] == $nomeDoUsuario){
                ...
                nomeCompleto, emailUsuario, senhaDoUsuario, dataCriar) 
                values(?, ?, ?, ?, ?)");
                $sql->bind_param("sssss", $nomeDoUsuario, $nomeCompleto,
                $emailUsuario, $senhaCodificada, $dataCriar);
                if($sql->execute()){
        ...
        $senha = sha1($senhaUsuario);//senha com codigo
    ...
    if($busca != null){
        ...
        echo "Falhou a entrada no sistema. Nome de usa´rio ou senha inválidos";
        echo "</p>";
        exit();
    }
